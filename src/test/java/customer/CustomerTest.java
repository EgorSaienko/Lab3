...
public class CustomerTest {
    ...
    public void testPrintCustomerDaysOverdrawn() throws Exception {
        ...
        assertThat(customer.printCustomerDaysOverdrawn(), is("danix dan account.Account: IBAN: RO023INGB434321431241, Days Overdrawn: 9"));
    }

    @Test
    public void testPrintCustomerMoney() throws Exception {
        Customer customer = getPersonWithAccount(false);
        assertThat(customer.printCustomerMoney(), is("danix dan account.Account: IBAN: RO023INGB434321431241, Money: 34.0"));
    }

    @Test
    public void testPrintCustomerAccountNormal() throws Exception {
        Customer customer = getPersonWithAccount(false);
        assertThat(customer.printCustomerAccount(), is("account.Account: IBAN: RO023INGB434321431241, Money: 34.0, account.Account type: normal"));
    }

    @Test
    public void testPrintCustomerAccountPremium() throws Exception {
        Customer customer = getPersonWithAccount(true);
        assertThat(customer.printCustomerAccount(), is("account.Account: IBAN: RO023INGB434321431241, Money: 34.0, account.Account type: premium"));
    }
    ...
    private Customer getPersonCustomer(Account account) {
        Customer customer = new PersonCustomer("danix", "dan", "dan@mail.com", account);
        account.setCustomer(customer);
        return customer;
    }

    private Customer getCompanyCustomer(Account account) {
        Customer customer = new CompanyCustomer("company", "company@mail.com", account, 0.50);
        account.setCustomer(customer);
        return customer;
    }
}
